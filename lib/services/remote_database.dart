...
Future<String?> uploadImage(XFile image) async {
  ...
  var uploadURL = WebRequest.uploadImage().path;
  try {
...
Future<UpdateResult<User>> updateUser(User user) async {
    ...
    int id = user.id;
    try {
      var url = WebRequest.updateUser(id.toString()).url();
...
Future<User?> fetchUser(String email) async {
    try {
      var url = WebRequest.fetchUser(email).url();
...
Future<bool> deleteAccount() async {
  try {
    ...
    if (id != null) {
       var url = WebRequest.deleteAccount(id.toString()).url();
